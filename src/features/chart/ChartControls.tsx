    ...
    <div className="border-b border-subtle bg-surface-2/50">
      <div className="px-6 py-4">
        <div className="flex flex-wrap items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              <span className="text-sm font-medium text-secondary">Show Series:</span>
              <InfoIcon
            ...
            <div className="flex gap-2">
              {([
                {key:'hodl', label:'Hold Bitcoin', color: 'border-orange-500', bgColor: 'bg-orange-50 dark:bg-orange-500/10'},
                {key:'buy', label:'Buy House', color: 'border-emerald-500', bgColor: 'bg-emerald-50 dark:bg-emerald-500/10'},
                {key:'rent', label:'Rent Forever', color: 'border-sky-500', bgColor: 'bg-sky-50 dark:bg-sky-500/10'},
                {key:'opportunity', label:'Opportunity Cost', color: 'border-red-500', bgColor: 'bg-red-50 dark:bg-red-500/10'},
              ] as const).map((s) => (
                <button
                  key={s.key}
                  onClick={()=> setState({ visibleSeries: { ...vis, [s.key]: !(vis as any)[s.key] } })}
                  className={`px-3 py-2 rounded-lg text-sm font-medium border-2 transition-all duration-200 focus-ring ${s.color} ${
                    (vis as any)[s.key]
